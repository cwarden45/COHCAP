...
\item Common annotations are pre-installed with the Bioconductor package.  For HM450k array, 27k array, and selected targeted BS-Seq data, you do not need
to provide an annotation file to COHCAP.
\item COHCAP output files are now better organized into subfolders for easier interpretation (compared to the standalone v1.2.4).
\item A filter for delta-beta values has been added in the Bioconductor package (compared to the standalone v1.2.4).
...
\begin{itemize}
\item Because COHCAP functions are now universal in the Bioconductor package, the COHCAP Bioconductor package doesn't filter Illumina array data based upon
detection p-values.  In most cases, lack of a detection p-value filter will probably not be a problem.  However, very poor quality data will show a high
proprotion of high detection p-values, so you may want to run additional QC analysis on your samples to ensure you are working with good quality data.
\item The Bioconductor pacakge doesn't contain a GUI.
\item There are minor formatting differences between the two COHCAP packages, so standalone documentation may not completely apply to the Bioconductor package.
\end{itemize}
\end{description}
...
<<island>>=
filtered.islands <- COHCAP.avg.by.island(sample.file, filtered.sites, beta.table,
...
<<integrate>>=
COHCAP.integrate.avg.by.island(filtered.islands, project.name, project.folder,
